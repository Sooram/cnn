import numpy as np
import matplotlib as mpl; mpl.use('TkAgg')  # to import pyplot
...
tf.flags.DEFINE_string("data_dir", "../data_emotions/", "Path to data files")
tf.flags.DEFINE_string("logs_dir", "../log_emotion_detector/", "Path to where log files are to be saved")

train_images, train_labels, valid_images, valid_labels, test_images = EmotionDetectorUtils.read_data(FLAGS.data_dir)
print "train images shape = ", train_images.shape   # (3761, 48, 48, 1)
print "test images shape = ", test_images.shape     # (1312, 48, 48, 1)

# check the first image 
image_0 = train_images[0]
label_0 = train_labels[0]
print "image_0 shape = ", image_0.shape # (48, 48, 1)
print "image_0 label = ", label_0   # [0. 0. 0. 1. 0. 0. 0.]
...
global_step = tf.Variable(0, trainable=False)   # 지금까지 수행된 최적화 반복 횟수
...
new_saver = tf.train.import_meta_graph('../log_emotion_detector/model.ckpt-1000.meta')
new_saver.restore(sess, '../log_emotion_detector/model.ckpt-1000')
tf.get_default_graph().as_graph_def()

x = sess.graph.get_tensor_by_name("input:0")
y_conv = sess.graph.get_tensor_by_name("output:0")

image_0 = test_images[0]
image_0 = np.resize(image_0, (1,48,48,1))
plt.imshow(image_0, cmap='Greys_r')
plt.show()
# image_0 = np.resize(gray,(1,48,48,1))
tResult = EmotionDetectorUtils.testResult()
num_evaluations = 10
